...
def init_layout(exclusion=None, frame_sz=(320, 240)):
    ...
    layout = [
        [sg.Text('Subject ID:', pad=((0, 0), 0), justification='left'), sg.Combo(meta.get_subj_ids(meta.get_conn(
        )), default_value="test", key='subj_id', size=(44, 1), background_color='white', text_color='black')],
        [space()],
        [sg.Text('Staff ID:', pad=((0, 0), 0), justification='left'), sg.Input(
            default_text="AN", key='staff_id', size=(44, 1), background_color='white', text_color='black')],
        [space()],
        [sg.T("Study ID"), sg.Combo(meta.get_study_ids(meta.get_conn()),
                                    key='study_id', enable_events=True, size=(44, 1))],
        [space()],
        [sg.T("Collection ID"), sg.Combo(
            "", key='collection_id', enable_events=True, size=(44, 1))],
        [space()],
        [sg.Text('Task combo: '), sg.Combo("", size=(64, 1), key="_tasks_")],
        [space()],
        [space(), sg.ReadFormButton('Save', button_color=(
...
def main_layout(sess_info, frame_sz=(320, 240)):
    ...
    for task in sess_info['_tasks_'].split(", "):
        ...
        field_tasks.append([space(), sg.Checkbox(
            name, key=key, size=(44, 1), default=True)])

    layout_col1 = [
        [sg.Text(f'Subject ID: {sess_info["subj_id"]}', pad=((0, 0), 0), justification='left', size=(25, 1)),
         sg.Text(f'Staff ID: {sess_info["staff_id"]}', pad=(
             (0, 0), 0), justification='left')
         ],

        [space()],
        [sg.Text('RC Notes:', pad=((0, 0), 0), justification='left'), sg.Multiline(
            key='notes', default_text='', size=(64, 10)), space()],
    ...
    layout_col2 = [[sg.Image(data=imgbytes, key='Webcam', size=frame_sz)],
                   ...
                   [sg.Text('', k="task_running",
                            justification='left', size=(20, 1))],
                   [space()], [space()], [space()], [space()],
                   [sg.Text('Inlet streams')],
                   [sg.Multiline(size=(35, 10), key='inlet_State',
                                 do_not_clear=False, no_scrollbar=True)]
                   ]
