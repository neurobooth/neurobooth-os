...
class DSC():
    ...
    def my_textbox2(self, text, pos=(0, 0), size=(None, None)):
        ...
        tbx = TextBox2(self.win, pos=pos, color='black', units='deg', lineSpacing=.9,
                       # size=(20, None),
                       letterHeight=1.1, text=text, font="Arial",
...
#        self.tmbUI["UIkeys"] = [keyToCode('1'),keyToCode('2'),keyToCode('3')]
    ...
    def onreadyUI(self, frame):
        ...
        if frame["type"] == "practice" or (frame["type"] == "test" and
            ...
            self.results.append({
                ...
                # the key or element chosen
                "response": self.tmbUI["response"][1],
                'correct': correct,  # boolean correct
                "rt": self.tmbUI["rt"],  # response time
                # response timestamp
                "timestamp": self.tmbUI["downTimestamp"],
                # state of the response handler
                "state": self.tmbUI["status"]
            })
        ...
        if frame["type"] == "practice":
            ...
            if self.tmbUI["status"] == "timeout" or not correct:
                ...
                message = [
                    visual.ImageStim(
                        self.win, image=frame["source"], pos=(0, 6), units='deg'),
                    visual.ImageStim(
                        self.win, image=self.fpath + '/DSC/images/key.png', pos=(0, 0), units='deg'),
    ...
    def nextTrial(self):
        ...
        while len(self.frameSequence):
            ...
            # deal with practice and test frames
                ...
                while countDown.getTime() > 0:
                    key = event.getKeys(
                        keyList=['1', '2', '3'], timeStamped=True)
                    if key:
                        kvl = key[0][0]
        ...
        df_out = pd.DataFrame.from_dict(
            self.outcomes, orient='index', columns=['vals'])
    ...
    def setFrameSequence(self):
        ...
        testMessage = {
            "begin": [
                ...
                self.my_textbox2(
                    'Press <b>[Continue]</b> for instructions', (0, -6)),

                visual.ImageStim(self.win, image=self.fpath +
                                 '/DSC/images/key.png', pos=(0, 0), units='deg')
            ],
            ...
            "practice": [
                [
                    ...
                    self.my_textbox2(
                        "Each <b>symbol</b> has a <b>number</b>", (0, -6)),
                ...
                [
                    visual.ImageStim(self.win, image=self.fpath +
                                     '/DSC/images/1.gif', pos=(0, 6), units='deg'),
                    ...
                    self.my_textbox2(
                        'Press <b>[Continue]</b> to continue', (0, -10)),
                ],
                [
                    visual.ImageStim(self.win, image=self.fpath +
                                     '/DSC/images/2.gif', pos=(0, 6), units='deg'),

                    visual.ImageStim(
                        self.win, image=self.fpath + '/DSC/images/keySmall.png', pos=(0, 0), units='deg'),
