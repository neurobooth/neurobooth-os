...
class DataInlet(Inlet):
    ...
    def __init__(self, info: pylsl.StreamInfo, plt: pg.PlotItem):
        ...
        empty = np.array([])
        # create one curve object for each channel/line that will handle
        # displaying the data
        self.curves = [
            pg.PlotCurveItem(
                x=empty,
    ...
    def pull_and_plot(self, plot_time):
        ...
        # ts will be empty if no samples were pulled, a list of timestamps
        # otherwise
        if ts:
            ts = np.asarray(ts)
...
def main():
    ...
    def scroll():
        ...
        for pw in plttrs.values():
            pw.setXRange(plot_time - plot_duration +
                         fudge_factor, plot_time - fudge_factor)

    def update():
        # Read data from the inlet. Use a timeout of 0.0 so we don't block GUI
        # interaction.
