...
class stream_plotter():
    ...
    def start(self, inlets):
        ...
        if self.plt_img is True and any(
                [k for k in ['Mouse', "mbient", "Audio"] if k in inlets.keys()]):
            self.thread_img = threading.Thread(
                target=self.update_imgs, daemon=True)
            self.thread_img.start()

        if self.plt_ts is True and any(
                [k for k in ['Mouse', "mbient", "Audio"] if k in inlets.keys()]):
            self.thread_ts = threading.Thread(
                target=self.update_ts, daemon=True)
            ...
            for nm, inlet in self.inlets.items():
                if nm in ['Marker', 'Markers']:
                    ...
                    if nm == "Mouse":
                        ax_ith = 0
                        clicks = [[tt, t[-1]]
                                  for t, tt in zip(tv, ts)if t[-1] != 0]
                        tv = [t[:-1] for t in tv]
                    elif nm == "mbient":
                        ax_ith = 1
                    ...
                    inlet.ydata = np.vstack(
                        (inlet.ydata[sz - buff_size:-1, :], tv))
                    inlet.xdata = np.hstack(
                        (inlet.xdata[sz - buff_size:-1], ts))
    ...
    def update_imgs(self):
        ...
        frame_screen, frame_cam = np.ones(
            (240, 320), dtype=np.uint8), np.ones((240, 320), dtype=np.uint8)
        ...
        while self.pltotting_img:
            for nm, inlet in self.inlets.items():
                ...
                if nm in ['Marker', 'Markers']:
                    ...
                    frame_screen = np.array(
                        tv, dtype=np.uint8).reshape(240, 320)
