...
class VidRec_Brio():
    def __init__(
      ...
      # sizex=640, sizey=480, fps=120, camindex=0, mode=19, doPreview=False):
        ...
        self.fps = fps
        # capture images (with no decrease in speed over time; testing is
        # required)
        self.fourcc = fourcc
        # scale_percent = 50
...
#        self.outlet.__del__()
    ...
    def prepare(self, name="temp_video"):
        self.video_filename = "{}_brio{}_{}.avi".format(
            name, self.device_index, time.time())
                ...
                if self.doPreview:
                    ...
                    if (self.frame_counter % self.preview_relFps) == 0:
                        ...
                        try:
                            ...
                            self.outlet_preview = StreamOutlet(
                                self.info_stream)
                            self.outlet_preview.push_sample(frame.flatten())
