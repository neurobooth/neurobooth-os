...
class DShowCapture():
    def __init__(self):
        ...
        if lib is None or bm_lib is None:
            if platform.architecture()[0] == '32bit':
                dll_path = resolve(os.path.join(
                    "dshowcapture", "dshowcapture_x86.dll"))
                lib = cdll.LoadLibrary(dll_path)
                dll_path = resolve(os.path.join(
                    "dshowcapture", "libminibmcapture32.dll"))
                bm_lib = cdll.LoadLibrary(dll_path)
            else:
                dll_path = resolve(os.path.join(
                    "dshowcapture", "dshowcapture_x64.dll"))
                lib = cdll.LoadLibrary(dll_path)
                dll_path = resolve(os.path.join(
                    "dshowcapture", "libminibmcapture64.dll"))
                bm_lib = cdll.LoadLibrary(dll_path)
            ...
            lib.capture_device.argtypes = [
                c_void_p, c_int, c_int, c_int, c_int]
            lib.capture_device_by_dcap.argtypes = [
                c_void_p, c_int, c_int, c_int, c_int, c_longlong]
    ...
    def capture_device_by_dcap(self, cam, dcap, width, height, fps):
        ...
        if self.info[cam]['type'] == "DirectShow":
            ret = self.lib.capture_device_by_dcap(
                self.cap, cam, dcap, width, height, fps) == 1
        ...
        if ret:
            ...
            if self.type == "DirectShow":
                ...
                self.fps = int(
                    10000000 / self.info[cam]['caps'][dcap]['minInterval'])
                self.flipped = True
                self.colorspace = 101
    ...
    def get_frame(self, timeout):
        ...
        if self.colorspace in [100, 101]:
            if self.real_size == self.height * self.width * 4:
                img = cv2.cvtColor(img.reshape(
                    (self.height, self.width, 4)), cv2.COLOR_BGRA2BGR)
            ...
            img = cv2.cvtColor(img.reshape(
                (3 * self.height // 2, self.width, 1)), cv2.COLOR_YUV2BGR_I420)
        elif self.colorspace == 201:
            img = cv2.cvtColor(img.reshape(
                (3 * self.height // 2, self.width, 1)), cv2.COLOR_YUV2BGR_NV12)
        elif self.colorspace == 202:
            img = cv2.cvtColor(img.reshape(
                (3 * self.height // 2, self.width, 1)), cv2.COLOR_YUV2BGR_YV12)
        elif self.colorspace == 203:
            img = cv2.cvtColor(img.reshape(
                (self.height, self.width, 1)), cv2.COLOR_GRAY2BGR)
        elif self.colorspace == 300:
            img = cv2.cvtColor(img.reshape(
                (self.height, self.width, 2)), cv2.COLOR_YUV2BGR_YVYU)
        elif self.colorspace == 301:
            img = cv2.cvtColor(img.reshape(
                (self.height, self.width, 2)), cv2.COLOR_YUV2BGR_YUY2)
        elif self.colorspace == 302:
            img = cv2.cvtColor(img.reshape(
                (self.height, self.width, 2)), cv2.COLOR_YUV2BGR_UYVY)
