...
class VidRec_Flir():
    ...
    def camCaptureVid(self):
        while self.recording or self.image_queue.qsize():
            # get images formated as numpy from separate process queue
    ...
    def prepare(self, name="temp_video"):
        ...
        # for some reason FFMPEG takes exponentially longer to write at
        # nonstandard frame rates, so just use default 25fps and change
        # elsewhere if needed
        ...
        # outputdict={'-r': str(self.FRAME_RATE_OUT), '-c:v': 'libx264',
        # '-crf': str(self.crfOut), '-preset': 'ultrafast'})#, , '-pix_fmt':
        # 'yuv444p'})#,
        ...
        # outputdict={'-r': str(fps), '-c:v': 'ffv1', '-crf': str(crf),
    ...
    def record(self):
        ...
        while self.recording:
            ...
            try:
                ...
                print(
                    f"Reopening FLIR {self.device_index} stream already closed")
                self.outlet = self.createOutlet(self.video_filename)
        ...
        print(
            f"FLIR recording ended with {self.frame_counter} frames in {time.time()-t0}")
