...
class VidRec_Flir():
    ...
    def prepare(self, name="temp_video"):
        ...
        # nonstandard frame rates, so just use default 25fps and change elsewhere
        # if needed
        ...
        # outputdict={'-r': str(self.FRAME_RATE_OUT), '-c:v': 'libx264', '-crf':
        # str(self.crfOut), '-preset': 'ultrafast'})#, , '-pix_fmt': 'yuv444p'})#,
        ...
        # outputdict={'-r': str(fps), '-c:v': 'ffv1', '-crf': str(crf), '-preset':
        # 'ultrafast', '-pix_fmt': 'yuv444p'})
