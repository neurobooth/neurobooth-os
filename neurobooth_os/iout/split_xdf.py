...
def split_sens_files(
    ...
    task_id: str,
    conn,
    video_files: Optional[dict] = None,
) -> List[str]:
    ...
    :param video_files: Mapping of stream name to file basenames, supplied by
        :class:`RecordingFiles` messages.
    ...
    device_data = parse_xdf(xdf_path, meta.get_device_ids(task_id), video_files)
...
def parse_xdf(
    ...
    for device_data in data:
        ...
        results.append(DeviceData(
            ...
            marker_data=marker,
            video_files=video_files.get(device_name, []),
...
def postpone_xdf_split(
    ...
    :param video_files: Mapping of stream name to file basenames (serialized as JSON).
    """
    video_json = json.dumps(video_files or {})
    with open(backlog_file, "a+") as f:
        f.write(f"{xdf_path},{task_id},{log_task_id},{video_json}\n")
...
def postprocess_xdf_split(
    ...
    with open(backlog_file, newline="") as csvfile:
        for row in csv.reader(csvfile, delimiter=",", quotechar="|"):
            ...
            # The JSON video_files dict contains commas, so the CSV reader
            # may have split it across multiple fields — rejoin them.
            video_json = ",".join(row[3:])
            video_files = json.loads(video_json) if video_json else None
            try:
                split_sens_files(xdf_path, log_task_id, task_id, conn, video_files=video_files)
            except:
