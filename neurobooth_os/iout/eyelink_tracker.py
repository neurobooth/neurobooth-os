...
class EyeTracker():
    ...
    def __init__(
        ...
        self.stream_info = StreamInfo(
            'EyeLink', 'Gaze', 20, self.sample_rate, 'float32', self.oulet_id)
    ...
    def connect_tracker(self):
        ...
        # Send a DISPLAY_COORDS message so Data Viewer knows the correct screen
        # size
        self.tk.sendMessage(
            f"DISPLAY_COORDS = 0 0 {self.mon_size[0]-1} {self.mon_size[1]-1}")

        # Choose a calibration type, H3, HV3, HV5, HV13 (HV =
        # horizontal/vertical)
    ...
    def calibrate(self):
        ...
        prompt_msg = visual.TextStim(
            self.win, text=prompt, color='Black', units='pix')
        prompt_msg.draw()
        self.win.flip()
        ...
        while self.recording:
            ...
            if smp is not None and old_sample != smp:
                ...
                if smp.isRightSample():
                    ...
                    values[:7] = [p for pp in [gaze, href, raw]
                                  for p in pp] + [pupil]
                if smp.isLeftSample():
                    ...
                    values[7:14] = [p for pp in [gaze, href, raw]
                                    for p in pp] + [pupil]
    ...
    def stop(self):
        ...
        if self.streaming:
            ...
            t0 = time.time()
            self.tk.receiveDataFile(
                self.fname_temp, f'{config.paths["data_out"]}{self.filename}')
