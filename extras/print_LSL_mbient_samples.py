...
for file in fl[9:]:
    ...
    duration = (
        data["device_data"]["time_stamps"][-1] - data["device_data"]["time_stamps"][0]
    )
    ...
    print(
        name_print,
        nsmpl,
        f"{duration :.2f}, fps duration:{int(nsmpl/duration)}, fps:{int(fps)}, fps local:{int(fps_local)}, fps mean:{int(fps_m)}",
    )
