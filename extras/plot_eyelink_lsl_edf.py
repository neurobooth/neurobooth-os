...
for f_asc in f_ascs: 
    ...
    smpl_valid = [i for i, v in enumerate(smpls) if not np.isnan(v[1]) and not np.isnan(v[3])]
    ...
    marker_stmps = [s for (s, t) in zip(marker_stmps, marker_ts) if '!V TARGET_POS' in t[0]]
    marker_ts = [t for  t in  marker_ts if '!V TARGET_POS' in t[0]]
    marker_ts =[[e[0].split("_")[-1],  e[0].split(", ")[0].split(" ")[-1],  e[0].split(", ")[1].split(" ")[0]] for e in marker_ts]
    ...
    print(f'  diff Eyelink and LSL time stamps Mean {np.mean(tdiff)}, max {np.max(tdiff)} min {np.min(tdiff)}')
    
    tdiff = events[:, 0] - marker_ts[:,0]
    print(f'  diff Eyelink and Unix time stamps Mean {np.mean(tdiff)}, max {np.max(tdiff)} min {np.min(tdiff)}')
    
    print( f'EDF FS: {np.median(1/np.diff(smpls[:,0]))}, LSL FS: {np.median(1/np.diff(dev_data_stmps))}')
    ...
    plt.plot(dev_data_stmps, dev_data_ts[:,0], alpha=.5, label='Gaze_R_x_LSL')
    plt.plot(smpls[:,0], smpls[:,3],  alpha=.5,label='Gaze_R_x_ET')
    plt.plot(dev_data_stmps, dev_data_ts[:,2], alpha=.5, label='Gaze_L_x_LSL')
    plt.plot(smpls[:,0], smpls[:,1],  alpha=.5,label='Gaze_L_x_ET')
