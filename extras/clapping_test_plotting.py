...
for ix, ky in enumerate(data.keys()):
    ...
    if "Mbient_RH" in ky:
        ax.plot(
            data[ky]["device_data"]["time_stamps"],
            1000 * data[ky]["device_data"]["time_series"][:, 1],
            color="red",
            label="acc_x",
        )
        ax.plot(
            data[ky]["device_data"]["time_stamps"],
            1000 * data[ky]["device_data"]["time_series"][:, 2],
            color="green",
            label="acc_y",
        )
        ax.plot(
            data[ky]["device_data"]["time_stamps"],
            1000 * data[ky]["device_data"]["time_series"][:, 3],
            color="blue",
            label="acc_z",
        )
        ...
        ax.plot(
            data[ky]["device_data"]["time_stamps"][1:],
            np.diff(data[ky]["device_data"]["time_series"][:, 0] * 10),
            "--",
            label="mbient t diff",
        )
