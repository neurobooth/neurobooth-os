...
path = r'Z:\data'
proc_data_path = r'Z:\processed_data'
session_id = '100059_2022-06-17'
# session_time = '_16h-50m-17s'
# session_id = '100064_2022-06-17'
session_time = '_18h-04m-53s'
...
legend_dict['Mbient_RH'] = ['acc_x', 'acc_y', 'acc_z']
...
for ix,ky in enumerate(data.keys()):
    ...
    if 'Mbient_RH' in ky:
        ax.plot(data[ky]['device_data']['time_stamps'], 1000*data[ky]['device_data']['time_series'][:,1], color='red', label='acc_x')
        ax.plot(data[ky]['device_data']['time_stamps'], 1000*data[ky]['device_data']['time_series'][:,2], color='green', label='acc_y')
        ax.plot(data[ky]['device_data']['time_stamps'], 1000*data[ky]['device_data']['time_series'][:,3], color='blue', label='acc_z')
        ## END of Mbient plotting

        ax.plot(data[ky]['device_data']['time_stamps'][1:], np.diff(data[ky]['device_data']['time_series'][:,0]*10), '--', label='mbient t diff')
